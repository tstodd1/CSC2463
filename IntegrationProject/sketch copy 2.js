...
let synthSaw = new Tone.Synth({
  ...
  //fx
  let delayFeedback;
  let delayAmt = new Tone.FeedbackDelay("8n", delayFeedback);
  synthSaw.connect(delayAmt);
  delayAmt.toDestination();

  function setDelayBasedOnDistance(distance) {
    let minDelay = 0.1;
    let maxDelay = 0.9;
    let minDistance = 0;
    let maxDistance = 200;

    let delayTime = map(distance, minDistance, maxDistance, minDelay, maxDelay);
    delayTime = constrain(delayTime, minDelay, maxDelay);
    // delayAmt.feedback.value = delayTime;
    delayFeedback = delayTime;
  }
  
  //pitches collections

  let midiNotes = [60, 67, 71, 74];
  let noteIndex;
  let midiVal, freq;
...
function startSound() {
    ...
    let index = noteIndex % midiNotes.length;
    // let midivVal = midiNotes[index];
    // let freq = Tone.Frequency(midiVal, "midi").toFrequency();
}
